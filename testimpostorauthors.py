...
for author in authors:
    # randomly pick a different author
    ainds = numpy.array(range(0,authors.shape[0]))
    ainds = numpy.delete(ainds,ind)
    otherauthor = ainds[random.randint(0,ainds.shape[0])]
    features = adata[adata[:,adata.shape[1]-1]==author,0:adata.shape[1]-1]
    otherfeatures = adata[adata[:,adata.shape[1]-1]==otherauthor,0:adata.shape[1]-1]
